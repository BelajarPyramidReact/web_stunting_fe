...
const ChildEdit = () => {
    ...
    useEffect(() => {
        ...
        const fetchChildData = async (id) => {
            try {
                setIsLoading(true);
                const response = await axios.get(`${process.env.REACT_APP_BACKEND_PUBLIC_URL}/children/${id}`);
        ...
        <div className="min-h-screen bg-gradient-to-b from-teal-50 to-white">
            ...
            <main className="max-w-4xl mx-auto py-12 px-4 sm:px-6 lg:px-8">
                <h1
                    ...
                    data-aos="fade-down"
                >
                    {child_id ? 'Edit Data Anak' : 'Pendaftaran Anak'}
                </h1>
                ...
                <div className="mb-8" data-aos="fade-up">
                    <div className="flex items-center justify-between">
                        {[1, 2, 3].map((item) => (
                            <div
                                key={item}
                                className={`w-1/3 text-center ${step === item ? 'text-teal-600 font-semibold' : 'text-gray-400'}`}
                            >
                                Step {item}
                            </div>
                        ))}
                    ...
                    <div className="flex h-2 mb-4">
                        <div className={`flex-1 ${step >= 1 ? 'bg-teal-500' : 'bg-gray-200'}`}></div>
                        <div className={`flex-1 ${step >= 2 ? 'bg-teal-500' : 'bg-gray-200'}`}></div>
                        <div className={`flex-1 ${step >= 3 ? 'bg-teal-500' : 'bg-gray-200'}`}></div>
                    </div>
                </div>
                ...
                >
                    ...
                    {isSuccess && (
                        <p className="text-center text-teal-600 font-semibold mt-4" data-aos="fade-up">
